...
4. 任意小任务节点完成后，必须进行编译，并根据编译结果进行验证与修复，编译正常后才可进行下一任务。
5. 按顺序完成任务，保持编辑最小化并专注于请求的变更。
6. 在更新状态之前确认完成——确保 `tasks.md` 中的每个项目都已完成。
7. 所有工作完成后更新检查清单，将每个任务标记为 `- [x]` 以反映实际情况。
8. 需要额外上下文时参考 `openspec list` 或 `openspec show <item>`。
